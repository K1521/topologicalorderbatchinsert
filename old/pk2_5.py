...
prt=0
rnodes,rconnections=makegraph(100000,4)
#rnodes,rconnections=['1', '4', '3', '0', '2'],[('1', '4'), ('0', '3'), ('1', '3'), ('4', '2')]
#rnodes,rconnections=["1", "2", "0"],[('0', '1'), ('0', '2'), ('1', '2')]

#if len(rnodes)<100:
if prt:print(rnodes,rconnections)
...
#ordinv=[(nametonode[name]:=Node(name,i)) for i,name in enumerate("arbsctduevfwgxhyizj")]#lol maby dont make everything a oneliner
...
if prt:print(newconnectionsptr)
...
if newconnectionsptr:
    newconnectionsptr.sort(key=lambda pse:-ord(pse[0]))

    lowerbound=len(ordinv)
    s=0
    for i,(x,y) in enumerate(newconnectionsptr):
        if ord(x)<lowerbound and i!=0:# or endwindow==-1:
            Q=discover(newconnectionsptr[s:i])
            Shift3(lowerbound,Q)
            s=i

        lowerbound=min(ord(y),lowerbound)
    Q=discover(newconnectionsptr[s:])
    Shift3(lowerbound,Q)
...
#Qs=[discover(B) for lb,B in Bs]
if prt:print(ordinv)
if prt:print([n.name for n in ordinv])
if prt:print(newconnections)
